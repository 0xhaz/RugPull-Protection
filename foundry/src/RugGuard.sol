...
/**
 * @title RugGuard
 * @notice A contract to protect against rug pulls in Uniswap V4 pools with intergration
...
contract RugGuard is BaseHook, Ownable, ReentrancyGuard, AutomationCompatibleInterface {
    ...
    /// @notice Maximum risk score for a pool
     ...
     * @param poolId The ID of the pool
     * @param riskScore The risk score of the pool
     */
    ...
    function afterAddLiquidity(
        ...
        PoolId poolId = key.toId();
        _updatePoolInfo(poolId, params.liquidityDelta);
    ...
    /**
     * @notice Hook called before a swap is executed in a pool
     * @param key The pool key
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        require(pool.riskScore < MAX_RISK_SCORE, "RugGuard: Pool is too risky for swaps");
        require(pool.totalLiquidity >= MIN_LIQUIDITY_THRESHOLD, "RugGuard: Insufficient liquidity in the pool");

        uint256 absAmountSpecified = uint256(params.amountSpecified);

        _updateVolume(poolId, absAmountSpecified);

        address token0 = Currency.unwrap(key.currency0);
        address token1 = Currency.unwrap(key.currency1);

        _updatePrice(poolId, token0, token1);

        return (BaseHook.beforeSwap.selector, BeforeSwapDeltaLibrary.ZERO_DELTA, 0);
    }

    /**
     * @notice Hook called after a swap is executed in a pool
     * @param key The pool key
     * @param params The swap parameters
     * @param delta The balance delta
     * @return The function selector
    ...
    function _handleLiquidityChange(PoolKey calldata key, int256 liquidityDelta) internal returns (bytes4) {
        ...
        _updatePoolInfo(poolId, liquidityDelta);
     ...
     * @param liquidityDelta The change in liquidity
     */
    function _updatePoolInfo(PoolId poolId, int256 liquidityDelta) internal {
        PoolInfo storage pool = poolInfo[poolId];
        ...
        _updateRiskScore(poolId, oldLiquidity, liquidityDelta);
     ...
     */
    function _updateRiskScore(PoolId poolId, uint256 oldLiquidity, int256 liquidityDelta) internal {
        PoolInfo storage pool = poolInfo[poolId];
        ...
        if (liquidityDelta < 0 && liquidityChangePercentage > 100) {
     ...
     */
    function _updateVolume(PoolId poolId, uint256 amountIn) internal {
        PoolInfo storage pool = poolInfo[poolId];
    ...
    /**
     * @notice Updates the price of a pool Chainlink price feeds
     * @param poolId The ID of the pool to update
     * @param token0 The address of the first token in the pool
     * @param token1 The address of the second token in the pool
     */
    function _updatePrice(PoolId poolId, address token0, address token1) internal {
     ...
     * @return performData The encoded pool ID to perform upkeep on
     */
     ...
     * @param performData The encoded pool ID to perform upkeep on
     */
